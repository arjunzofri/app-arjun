...
export default async function SalidasPage() {
  ...
  // Q2 + Q3: bodegas y modulos en paralelo
  const [bodegasData, modulosData] = await Promise.all([
    s`SELECT id, nombre FROM public.bodegas ORDER BY nombre`
      .then(rows => rows.map((r: any) => ({ id: r.id, nombre: r.nombre }))),
    s`SELECT id, nombre FROM public.modulos_destino ORDER BY nombre`
      .then(rows => rows.map((r: any) => ({ id: r.id, nombre: r.nombre }))),
  ]);

  // Q4: stock por bodega
  const productoIds = productosRows.map((p: any) => p.id);
  const stockMap = new Map<string, { bodegaId: string; cantidadActual: number }[]>();
  ...
  // Q5: imagenes (primera por producto)
  const imagenMap = new Map<string, any[]>();
  if (productoIds.length > 0) {
    ...
    for (const row of imgRows) {
      if (!imagenMap.has(row.producto_id as string)) {
