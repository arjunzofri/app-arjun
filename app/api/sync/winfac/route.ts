  ...
  for (const row of rows) {
    try {
      ...
      if (!prod) {
        ...
        if (!bodegaId || !adminId) {
          debugDetalle.push({ knumezet, accion: "ignorado", razon: "falta bodega o admin user" })
          continue
        }
        ...
        debugDetalle.push({ knumezet, accion: "creado", razon: `producto nuevo, bodega=${bodegaId}, vendedor_rut=${vendedorRut}, stock=${stocdispNum}` })
      } else {
        // === PRODUCTO EXISTENTE ===
